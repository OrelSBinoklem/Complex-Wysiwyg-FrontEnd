    ...
    else if(re_http.exec(url_1) !== null)
    {
        ...
        //Не наш хост
            ...
            if(re_http.exec(url_2) !== null)
            {
                ...
                {
                    return shablonizator.tolko_otnositelnii_put_v_rezultate('sb.html', url_2, true);
                }
            }
            var base_url_papki = shablonizator.tolko_otnositelnii_put_v_rezultate.papki_array(base_url_no_host[2]);
            var base_url_papki_conkat = '';
            ...
            if(result == 'other_domain')
            {
                ...
                //Если относительный
                    ...
                    if(url_1_papki.length >= url_2_kol_vozvratov)
                    {
                        ...
                        throw new Error('Ошибка в sb.adres_faila_iz_faila_otnositelno_tretego_faila');
                        alert('Ошибка в sb.adres_faila_iz_faila_otnositelno_tretego_faila');
                    }
                }
            }
        }
...
shablonizator.tolko_otnositelnii_put_v_rezultate = function(url_1, url_2, min_length)/*Зделать ещё одну функцию чтоб url_1 был любым (не только относительным)*/ {
    ...
    if(re_slesh.exec(url_2) !== null)
    {
        ...
        else {throw new Error('Ошибка в sb.tolko_otnositelnii_put_v_rezultate'); alert('Ошибка в sb.tolko_otnositelnii_put_v_rezultate');}
    }
    //Если в начале http
    ...
    //если url_2 просто относительный
        ...
        if(min_length)
        {
            ...
            if(base_url_no_host !== null)
            {
                ...
                else {throw new Error('Ошибка в sb.tolko_otnositelnii_put_v_rezultate'); alert('Ошибка в sb.tolko_otnositelnii_put_v_rezultate (возвратов больше чем папок в base_url)');}
            }
            else {throw new Error('Ошибка в sb.tolko_otnositelnii_put_v_rezultate'); alert('Ошибка в sb.tolko_otnositelnii_put_v_rezultate');}
        }
        ...
        throw new Error('Ошибка в sb.tolko_otnositelnii_put_v_rezultate');        
        return false;
    }
}
